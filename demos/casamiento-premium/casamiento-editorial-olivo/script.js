// =====================================================
//  Editorial Olivo · Plan Premium · script.js
//  Formulario integrado - conectar submitUrl en config.json
//  para enviar a Google Apps Script cuando esté listo.
// =====================================================
...
function init(c) {
  ...
  initGalleryLightbox();
  initMusica();
  RsvpModal.init(c.rsvp, c.pareja?.display);
}
...
function gcSetPos(card, pos, animate) {
  ...
  if (animate) {
    card.style.transition = 'transform 0.38s cubic-bezier(0.25,0.1,0.25,1), opacity 0.3s ease';
  } else {
...
function gcGoNext() {
  ...
  if (outCard) {
    outCard.style.transition = 'transform 0.42s cubic-bezier(0.55,0,1,0.45), opacity 0.32s ease';
...
function gcGoPrev() {
  ...
  requestAnimationFrame(() => requestAnimationFrame(() => {
    if (inCard) {
      inCard.style.transition = 'transform 0.42s cubic-bezier(0.25,0.1,0.25,1), opacity 0.3s ease';
...
function gcInitDrag(container) {
  container.addEventListener('pointerdown', e => {
    ...
    GC.dragging = true; GC.didDrag = false;
    GC.dragStartX = e.clientX; GC.dragDelta = 0;
...
function gcDragUpdate() {
  ...
  active.style.transform  =
    `translate3d(${d}px, 0, 0) rotateZ(${(d < 0 ? -1 : 1) * progress * 7}deg)`;
  if (d < 0) {
    const next = gcCard((GC.current + 1) % GC.total);
    if (next) {
      const st = GC_STACK[1];
      next.style.transition = 'none';
      next.style.transform  =
        `translate3d(${st.x * (1 - progress)}%, 0, ${st.z * (1 - progress)}px) rotateZ(${st.r * (1 - progress)}deg)`;
      const sh = next.querySelector('.gallery-card__shadow');
...
function renderGallery(galleryConfig) {
  ...
  GC.images = images; GC.total = images.length; GC.current = 0; GC.busy = false;
